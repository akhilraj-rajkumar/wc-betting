...
export class AddBetService extends BaseService {
  ...
  addBet(match: MatchModel, options) {
    const url = 'match/bid';
    const params = {
      match_id: match.id,
      match_bids: [
        { team_id: match.homeTeam.id, number_of_bids: options['homeTeam'] },
        { team_id: match.awayTeam.id, number_of_bids: options['awayTeam'] },
        { team_id: 2, number_of_bids: options['draw'] }
      ]
    }
    this.postData(url, params).subscribe(
      res => {
        ...
        if (responseStatus === SUCCESS_STATUS) {
          this.store.dispatch(new BetAddedSuccessAction({}));
        } else {
